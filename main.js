...
function crearTorneo(event) {
    ...
    if (esDispositivoMovil) {
        info.textContent = 'Preisona sobre un jugador para marcarlo como ganador y avanzar a la siguiente ronda. Mantén presionado para revertir un enfrentamiento.';
    } else {
    ...
    for (let i = 0; i < cantidad_jugadores; i++) {
        const indice = Math.floor(Math.random() * jugadores_sin_ordenar.length);
...
function sortearCartas(event) {
    ...
    function barajarCartas() {
        ...
        timer = requestAnimationFrame(cambiarCartas);
        // Barajamos durante 3 segundos
        setTimeout(() => {
            cancelAnimationFrame(timer);
            mostrarCartasFinales();
        }, 3000);
    }
    ...
    function mostrarCartasFinales() {
        ...
        setTimeout(ocultarBarraDeCarga, 500);
    }
}
